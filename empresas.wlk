class Empresa{
	...
	method esDeGenteAcotada(){
		...
		p=>p.provinciasDondeTrabaja().size()<=3
		})
	}
	...
	method esProfesionalPocoAtractivo(unProfesional){
		...
		const profeMismasProv = profesionales.filter({p=>p.provinciasDondeTrabaja().asSet().intersection(provinciasProf)})
		return profeMismasProv.any({p=>p.honorarios() < unProfesional.honorarios()})
	}
}